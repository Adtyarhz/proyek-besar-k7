...
<head>
    <meta charset="UTF-8">
    <title>Tabel Eligible KP - Koordinator</title>
    ...
    <tbody>
      @foreach($pendaftaranKPs as $index => $p)
        ...
        <tr>
          ...
          <td>
            ...
            <div id="form-catatan-koordinator-{{ $p->id }}" class="mt-2" style="display: none;">
              <form action="{{ route('koordinator.tableeligiblekp.update_catatan', $p->id) }}" method="POST">
                @csrf
                <textarea name="catatan_koordinator_eligible" class="form-control" rows="3" required>{{ $p->catatan_koordinator_eligible }}</textarea>
          ...
          <td>
            ...
            <div id="form-sks-koordinator-{{ $p->id }}" class="mt-2" style="display: none;">
              <form action="{{ route('koordinator.tableeligiblekp.update_sks', $p->id) }}" method="POST">
                @csrf
                <input type="number" name="sks_koordinator" class="form-control" value="{{ $p->sks_koordinator ?? 0 }}" required>
                <button type="submit" class="btn btn-primary btn-sm mt-2">Save</button>
              </form>
            </div>
            @if($p->sks_koordinator !== null)
          ...
          <td>
            @if($currentStatus == 'Menunggu')
              <form action="{{ route('koordinator.tableeligiblekp.update_status', $p->id) }}" method="POST" style="display:inline">
                @csrf
